function [final_period,period_distribution] = Autocorr_infra_periods(signal_trace, fs_ref, analysis_periods, varargin)
...
addParameter(p, 'MinPeakProminence', 0.15);
addParameter(p, 'SmoothingFactor', 1.5);
addParameter(p, 'MinPeakDistance', 1);
addParameter(p, 'MinPeakHeight', 0);
addParameter(p, 'PlotIndividual', false);
parse(p, varargin{:});
params = p.Results;
