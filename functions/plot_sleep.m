...
%   h: the output is a plot, and no argument is needed
    ...
    if nargin >= 6
        patch([sleepscore_time fliplr(sleepscore_time)], [ymin+zeros([1, length(REM_binary)]) ymin+(ymax-ymin)*fliplr(REM_binary)], [0.6157 0.7490 0.5412], 'EdgeColor', 'none', 'FaceAlpha', 0.5);
    end
    if nargin == 7
